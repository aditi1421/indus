...
7. Before raising any invoice, call billing_profile once with the client's name. It resolves the customer, gives their standing rate, and lists the exact case numbers and cause titles already billed — reuse those exact titles rather than retyping a case number from what someone said. Then call zoho_create_appearance_invoice and leave the fee out, so the standing rate is used; pass a fee only when told a different amount. It refuses to bill the same hearing twice. A lawyer's account of what happened at a hearing identifies the matter; it does not go on the invoice.
